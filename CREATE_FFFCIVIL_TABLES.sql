...
CREATE TABLE IF NOT EXISTS fffcivil_exam_results (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT unique_fffcivil_exam_attempt UNIQUE (roll_number),
    CONSTRAINT fk_fffcivil_student FOREIGN KEY (roll_number) 
        REFERENCES fffcivil_students(roll_number) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_fffcivil_exam_results_roll_number
ON fffcivil_exam_results(roll_number);

CREATE INDEX IF NOT EXISTS idx_fffcivil_exam_results_created_at
ON fffcivil_exam_results(created_at DESC);
...
-- 4. RLS POLICIES
-- =========================================================
DROP POLICY IF EXISTS "Allow public select fffcivil students" ON fffcivil_students;
CREATE POLICY "Allow public select fffcivil students" ON fffcivil_students
FOR SELECT TO anon, authenticated USING (is_active = true);

DROP POLICY IF EXISTS "Students can view own result" ON fffcivil_exam_results;
CREATE POLICY "Students can view own result" ON fffcivil_exam_results
FOR SELECT TO anon, authenticated
USING (roll_number IN (SELECT roll_number FROM fffcivil_students WHERE is_active = true));

DROP POLICY IF EXISTS "Allow insert if no prior attempt" ON fffcivil_exam_results;
CREATE POLICY "Allow insert if no prior attempt" ON fffcivil_exam_results
FOR INSERT TO anon, authenticated
WITH CHECK (roll_number IN (SELECT roll_number FROM fffcivil_students WHERE is_active = true));

-- =========================================================
-- 5. SECURE RPC FUNCTIONS
-- =========================================================
DROP FUNCTION IF EXISTS check_fffcivil_exam_eligibility(TEXT, TEXT);
CREATE OR REPLACE FUNCTION check_fffcivil_exam_eligibility(p_roll_number TEXT, p_unique_code TEXT)
...
BEGIN
    SELECT EXISTS(SELECT 1 FROM fffcivil_students WHERE roll_number = p_roll_number AND unique_code = p_unique_code AND is_active = true) INTO v_student_exists;
...
BEGIN
    ...
    BEGIN
        INSERT INTO fffcivil_exam_results (roll_number, correct_answers, wrong_answers, total_questions, percentage, user_answers, additional_data, violation_type, violation_detected)
...
-- =========================================================
-- 6. INSERT STUDENTS (3 students)
-- =========================================================
INSERT INTO fffcivil_students (roll_number, unique_code, is_active) VALUES
('160524732002', 'fffcivil', true), ('160524732010', 'fffcivil', true), ('160524732301', 'fffcivil', true)
ON CONFLICT (roll_number) DO UPDATE SET unique_code = EXCLUDED.unique_code, is_active = EXCLUDED.is_active;

-- =========================================================
-- 7. GRANT PERMISSIONS
-- =========================================================
GRANT EXECUTE ON FUNCTION check_fffcivil_exam_eligibility(TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION submit_fffcivil_exam_result(TEXT, INTEGER, INTEGER, INTEGER, NUMERIC, JSONB, JSONB, TEXT, BOOLEAN) TO anon, authenticated;

-- =========================================================
-- 8. VERIFICATION
-- =========================================================
SELECT COUNT(*) AS total_fffcivil_students FROM fffcivil_students;
SELECT * FROM check_fffcivil_exam_eligibility('160524732002', 'fffcivil');

...
-- 1. STUDENTS TABLE
-- =========================================================
CREATE TABLE IF NOT EXISTS fs4cse_students (
...
CREATE INDEX IF NOT EXISTS idx_fs4cse_students_roll_number
ON fs4cse_students(roll_number);

-- =========================================================
-- 2. EXAM RESULTS TABLE
-- =========================================================
CREATE TABLE IF NOT EXISTS fs4cse_exam_results (
    id BIGSERIAL PRIMARY KEY,
    ...
    CONSTRAINT fk_fs4cse_student FOREIGN KEY (roll_number) 
        REFERENCES fs4cse_students(roll_number) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_fs4cse_exam_results_roll_number
ON fs4cse_exam_results(roll_number);

CREATE INDEX IF NOT EXISTS idx_fs4cse_exam_results_created_at
ON fs4cse_exam_results(created_at DESC);

-- =========================================================
-- 3. ENABLE RLS (Row Level Security)
-- =========================================================
ALTER TABLE fs4cse_students ENABLE ROW LEVEL SECURITY;
ALTER TABLE fs4cse_exam_results ENABLE ROW LEVEL SECURITY;

-- =========================================================
-- 4. RLS POLICY: Students table - Read Only for Validation
-- =========================================================
DROP POLICY IF EXISTS "Allow public select fs4cse students" ON fs4cse_students;

CREATE POLICY "Allow public select fs4cse students"
ON fs4cse_students
FOR SELECT
...
-- =========================================================
DROP POLICY IF EXISTS "Students can view own result" ON fs4cse_exam_results;

CREATE POLICY "Students can view own result"
ON fs4cse_exam_results
FOR SELECT
TO anon, authenticated
USING (
    roll_number IN (
        SELECT roll_number FROM fs4cse_students WHERE is_active = true
    )
);
...
-- =========================================================
DROP POLICY IF EXISTS "Allow insert if no prior attempt" ON fs4cse_exam_results;
...
-- =========================================================
DROP FUNCTION IF EXISTS check_fs4cse_exam_eligibility(TEXT, TEXT);

CREATE OR REPLACE FUNCTION check_fs4cse_exam_eligibility(
    p_roll_number TEXT,
    p_unique_code TEXT
)
RETURNS TABLE(
...
BEGIN
    ...
    SELECT EXISTS(
        SELECT 1 FROM fs4cse_exam_results 
        WHERE roll_number = p_roll_number
...
BEGIN
    ...
    BEGIN
        INSERT INTO fs4cse_exam_results (
...
-- =========================================================
-- 9. INSERT STUDENTS (2 students)
-- =========================================================
INSERT INTO fs4cse_students (roll_number, unique_code, is_active)
VALUES
    ('160524733016', 'fs4cse', true),
    ('160524733006', 'fs4cse', true)
...
-- =========================================================
GRANT EXECUTE ON FUNCTION check_fs4cse_exam_eligibility(TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION submit_fs4cse_exam_result(TEXT, INTEGER, INTEGER, INTEGER, NUMERIC, JSONB, JSONB, TEXT, BOOLEAN) TO anon, authenticated;

-- =========================================================
-- 11. VERIFICATION QUERIES
-- =========================================================
-- Check students
SELECT COUNT(*) AS total_fs4cse_students FROM fs4cse_students;

SELECT roll_number, unique_code, is_active, created_at
FROM fs4cse_students
ORDER BY roll_number;

-- Check results
SELECT COUNT(*) AS total_fs4cse_results FROM fs4cse_exam_results;

SELECT roll_number, correct_answers, wrong_answers, percentage, 
       violation_detected, violation_type, created_at
FROM fs4cse_exam_results
...
-- =========================================================
-- Exam Code: fs4cse
-- Question Bank: thirdcse-questions.js (200 total, 20 random)
-- Students: 2
-- Duration: 30 minutes
